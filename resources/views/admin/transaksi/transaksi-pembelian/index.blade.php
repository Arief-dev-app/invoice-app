...
@section('content')
    @include('admin.transaksi.transaksi-pembelian.form')
@endsection
...
<script>
    ...
    const formPo = document.getElementById('po_select');   
    ...
    function renderDetailItems(details) {
        ...
        details.forEach((item) => {
            ...
            const hargaAngka = parseInt(item.harga); // hilangkan .00 agar tidak jadi 50 juta
            const hargaFormat = formatRupiah(String(hargaAngka));
            ...
            row.innerHTML = `
                ...
                <td class="border px-2 py-1">
                    <input type="number" name="items[${rowCount}][qty]" value="${item.qty}" class="w-full border rounded px-2 py-1">
                </td>
                
                <td class="border px-2 py-1 relative">
                    <input 
                        type="text" 
                        class="w-full border rounded px-2 py-1 format-harga" 
                        value="${hargaFormat}" 
                        data-index="${rowCount}">
                    <input 
                        type="hidden" 
                        name="items[${rowCount}][harga]" 
                        class="harga-angka" 
                        id="harga-hidden-${rowCount}" 
                        value="${hargaAngka}">
                </td>
            `;
    ...
    async function openGlobalModal(title = 'Tambah {{ $title }}', data = null, mode = 'create') {
        ...
        if (mode === 'edit' && data) {
            ...
            formTrans.value = data.purchase_no ? data.purchase_no : '';
            formTransDate.value = data.transaction_date ? data.transaction_date : '';
            formSupplier.value = data.supplier_id ? data.supplier_id : '';
            formPo.value = data.po_id ? data.po_id : '';

            formTrans.readOnly = true;
            formTransDate.readOnly = false;
            formSupplier.readOnly = false;
            formPo.readOnly = false;
    ...
    function addProductRow() {
        ...
        row.innerHTML = `
            ...
            <td class="border px-2 py-1">
                <input type="number" name="items[${rowCount}][harga]" value="0" class="w-full border rounded px-2 py-1 format-harga">
            </td>
    ...
    function resetForm() {
        form.reset(); // reset semua input standar (termasuk hidden)

        // Reset manual untuk input yang tidak ikut reset seperti select
        formTrans.value = '';
        formTransDate.value = '';
        formSupplier.value = '';
        formPo.value = '';

        // Reset product detail table
        const detailTbody = document.querySelector('#productDetailTableBody');
        detailTbody.innerHTML = '';
        rowCount = 0;

        // Reset readonly dan visibility
        ...
        data.data.forEach((data, index) => {
            html += `
                <tr>
                    ...
                    <td class="p-2 border">${(data.purchase.purchase_no)}</td>
