            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="border px-2 py-1 space-x-2">
                        <button onclick="fetchMenuAndOpenModal({{ $user->id }}, 'view')">👁</button>
                        <button onclick="fetchMenuAndOpenModal({{ $user->id }}, 'edit')">✏️</button>
                        <button onclick="handleDeleteAction(() => deleteMenu({{ $user->id }}))">🗑</button>
...
<script>
    ...
    async function fetchMenuAndOpenModal(menuId, mode = 'edit') {
        ...
        try {
            const response = await fetch(`user/${menuId}/edit`);
    ...
    async function deleteMenu(menuId) {
        ...
        try {
            const response = await fetch(`/user/${menuId}`, {
                method: 'POST',
                headers: {
    ...
    document.getElementById('menu_select').addEventListener('change', async function () {
        ...
        try {
            const response = await fetch(`/user/${groupId}/menus`);
