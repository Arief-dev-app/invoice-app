<div class="px-4 py-6">
    <div class="bg-white shadow rounded p-6">
        <div class="flex justify-between items-center mb-4">
            ...
            <!-- Bungkus kedua tombol dalam satu div agar mereka berdekatan -->
            <div class="flex gap-2">
                <button onclick="handlePrintExcel()" style="background-color: #16a34a;" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">
                    Cetak Excel
                </button>
                <button onclick="submitPdfForm()" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700">
                    Cetak PDF
                </button>
            </div>

            <form id="pdfForm" method="GET" action="{{ url('/laporan-penjualan/print-pdf') }}" target="_blank" class="hidden">
                <input type="hidden" name="start" id="pdfStartDate">
                <input type="hidden" name="end" id="pdfEndDate">
            </form>
        </div>
        ...
        <div class="flex items-end gap-4 mb-4">
            <div>
                <label for="formStartDate" class="block text-sm font-medium">Trans Date</label>
                <input 
                    type="date" 
                    name="trans_date" 
                    id="formStartDate" 
                    class="w-64 text-xs border rounded px-3 py-2" 
                    value="{{ date('Y-m-01') }}"
                >
            </div>

            <div>
                <label for="formEndDate" class="block text-sm font-medium">Due Date</label>
                <input 
                    type="date" 
                    name="due_date" 
                    id="formEndDate" 
...
<div id="globalModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white p-6 rounded shadow-lg w-full max-w-5xl h-[90vh] flex flex-col">
            ...
            <form id="productForm" method="POST">
                ...
                <div class="mb-4">
                        <div class="flex justify-between items-center mb-2">
                            <label for="user_group_select" class="block text-sm font-medium">Customer</label>
                        </div>
                        <select id="customer_select" name="customer_id" class="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
        ...
        <div class="flex justify-end space-x-2" id="formButtons">
            <button id="btnConfirm" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Confirm</button>
